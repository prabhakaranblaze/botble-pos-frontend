...
use Botble\Quickbooks\Listeners\QuickbookJobListener;
...
class OrderController extends BaseController
{
    ...
    public function destroy(Order $order)
    {
        return DeleteResourceAction::make($order)
            ->beforeDeleting(function ($action) use ($order) {

            // Check setting before deleting in QuickBooks
            $deleteSetting = Setting::get('quickbooks_sales_receipt_delete', 0);

            // If setting is OFF, don't delete
            if ($deleteSetting != 1) {
                \Log::info("QuickBooks SalesReceipt delete disabled in settings.");
                return;
            }

            // Setting is ON → proceed with deletion
            if ($order->quickbooks_sales_receipt_id) {
                try {
                    app(QuickbookJobListener::class)
                        ->deleteSalesReceipt($order->quickbooks_sales_receipt_id, $order->id);

                } catch (\Exception $e) {
                    \Log::error("Failed to delete QuickBooks SalesReceipt: " . $e->getMessage());
                }
            }

    ...
    @foreach ($groupedProducts as $grouped)
        ...
        <div class="mt-3 bg-light mb-3">
            ...
            @if ($isAvailableShipping && MarketplaceHelper::isChargeShippingPerVendor())
                <div class="shipping-method-wrapper py-3 {{$deliveryOption}}" 
                    @style(['display: none' => (bool) get_ecommerce_setting('disable_shipping_options', false) || ($deliveryOption === 'pickup')])
                >
                    @if (!empty($shipping))

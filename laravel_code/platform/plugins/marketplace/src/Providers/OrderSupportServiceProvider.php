...
class OrderSupportServiceProvider extends ServiceProvider
{
    ...
    public function handleCheckoutOrderByStore(
        ...
        // ===== Update Shipping Address for Pickup =====
        if ($request->delivery_option == 'pickup') {

            $shippingAddress = OrderAddress::where('order_id', $order->id)
                            ->where('type', OrderAddressTypeEnum::SHIPPING)
                            ->first();

            $billingAddress = OrderAddress::where('order_id', $order->id)
            ...
            if (! $billingAddress && $shippingAddress) {
                ...
                $billingAddress->type = OrderAddressTypeEnum::BILLING;
                $billingAddress->save();
            }
            ...
            if ($shippingAddress) {
                ...
                $shippingAddress->address = trans('plugins/pos-pro::pos.pickup_at_store');
                $shippingAddress->zip_code = null;
